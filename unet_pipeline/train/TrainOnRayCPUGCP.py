# python TrainOnRayCPUGCP.py experiments/albunet_valid/train_config_part0.yaml --gcp_bucket img-classif-training-dataset
...
def argparser():
    parser = argparse.ArgumentParser(description='Pneumatorax pipeline on Ray')  # Updated description
...
def main():
    ...
    # GCP bucket and remote directory (Define these here)
    bucket_name = args.gcp_bucket  # Get bucket name from command-line argument
    remote_data_dir = "dataset/dataset1024"  #  Remote directory in GCP

    # Resolve the local data directory from config
    local_data_dir = Path(train_config['DATA_DIRECTORY']).resolve()

    # Copy data from GCP if local directory doesn't exist
    if not check_local_directory_exists(local_data_dir, remote_data_dir):
        print(f"Downloading dataset from GCP bucket '{bucket_name}'...")
        copy_folder_from_gcp(bucket_name, remote_data_dir, local_data_dir)
    else:
        print(f"Dataset already exists locally at: {local_data_dir}")


    # Update the data directory in the config to use the local path (important!)
    train_config['DATA_DIRECTORY'] = str(local_data_dir)
    print(f"Using dataset from local directory: {train_config['DATA_DIRECTORY']}")
    ...
    for fold_id in usefolds:
        ...
        # Start training remotely using Ray
        future = train_fold.remote(  # Use ray.remote here
            train_config, experiment_folder, pipeline_name, log_dir, fold_id,
    ...
    ray.get(futures) # Wait for all Ray tasks to complete
    ray.shutdown() # Shutdown Ray

    # Upload results to GCP bucket (assuming gcp_handler is defined elsewhere - if not, you'll need to implement or remove this part)
    # gcp_handler.upload_folder(log_dir, f"results/{pipeline_name}") # You might need to define gcp_handler or replace this
