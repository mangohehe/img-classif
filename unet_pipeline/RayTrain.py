...
@ray.remote
def ray_list_files(bucket_name):
    """
    Ray remote function to list files in a GCP bucket.
    ...
    Returns:
        list: List of file paths in the bucket.
    """
    return list_files_in_bucket(bucket_name)


def download_file_from_bucket(bucket_name, remote_path, local_path):
    """
...
def main():
    ...
    # List files in the GCP bucket using Ray
    bucket_name = args.gcp_bucket
    files = ray.get(ray_list_files.remote(bucket_name))
    ...
    for file in files:
        if file.startswith(remote_data_dir):
            local_path = local_data_dir / \
                Path(file).relative_to(remote_data_dir)
            download_file_from_bucket(bucket_name, file, local_path)
